...
test('useSettingsState initial value', async () => {
  ...
  jest.mocked(mockRepository.requestSettings).mockImplementation((): Promise<Settings> => {
...
test('useOnSubmit', async () => {
  ...
  const { result } = renderHook(() => useOnSubmit(settingsState, mockRepository))
  ...
  expect(mockRepository.postSettings).toBeCalledWith(expected)
