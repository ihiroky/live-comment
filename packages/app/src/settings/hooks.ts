...
export function useSettingsState(repository: SettingsRepository): SettingsState {
  ...
  useEffect((): void => {
    repository.requestSettings().then((settings: Settings): void => {
...
export function useOnSubmit(
  ...
  return useCallback((e: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    return repository.postSettings(settings).then(() => window.close())
  }, [settingsState, repository])
}
